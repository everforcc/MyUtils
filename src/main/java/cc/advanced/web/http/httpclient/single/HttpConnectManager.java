package cc.advanced.web.http.httpclient.single;
...
public class HttpConnectManager {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 4. 将http://www.baidu.com:80的最大连接增加到50
    ...
    public static void doGet(HttpClientConnectionManager connectionManager,String url) throws Exception {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                System.out.println("内容长度：" + content.length()  + "---" + content);
            }
        } finally {
            if (response != null) {
                response.close();
    ...
    public static class IdleConnectionEvictor extends Thread {
        ...
        public void run() {
            try {
                while (!shutdown) {
                    System.out.println("清理无效连接:" + connectionManager.toString());
                    synchronized (this) {
                        //3s检查一次
                        wait(3000);
                        // 关闭失效的连接
