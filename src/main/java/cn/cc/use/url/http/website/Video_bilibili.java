...
public class Video_bilibili {
    ...
    public static void t4(String url)throws Exception{
        ...
        if(httpEntity != null) {
            ...
            InputStreamUtils.downFileByStream(httpEntity.getContent(),"E:\\java\\test","3.flv");
            //FileUtils.copyToFile(httpEntity.getContent(),new File("E:\\java\\test\\2.flv"));
        }
        // buffer 包裹一下更快
        response.close(); // 关闭流
        httpClient.close();
    ...
    public static void requestHeard_downFlv(String flvUrl) throws Exception {
        ...
        // 测试 io速度 网址的 缓存写法
        FileUtils.copyToFile(conn.getInputStream(),new File("E:\\java\\test\\ep336055.flv"));// 23:55
    }
